...
class Config:
    ...
    DEFAULT_CONFIG = {
        ...
        "mqtt": {
            ...
            "tls_keyfile": None,
            "tls_insecure": False,
            "tls_version": "CLIENT_DEFAULT",  # Options: "TLSv1.2", "TLSv1.3", "CLIENT_DEFAULT" (for auto-negotiate)
    ...
    def load_config(self):
        ...
        if not loaded_path:
            logger.info("No config.json found, using default configuration. Consider creating a config.json.")
    ...
    def _override_from_env(config_data):
        ...
        config_data['database']['port'] = int(os.environ.get('DB_PORT', config_data['database']['port']))
        ...
        config_data['database']['password'] = os.environ.get('DB_PASSWORD', config_data['database']['password'])
        
        # MQTT configuration
        config_data['mqtt']['broker_host'] = os.environ.get('MQTT_BROKER_HOST', config_data['mqtt']['broker_host'])
        config_data['mqtt']['broker_port'] = int(os.environ.get('MQTT_BROKER_PORT', config_data['mqtt']['broker_port']))
        config_data['mqtt']['username'] = os.environ.get('MQTT_USERNAME', config_data['mqtt']['username'])
        config_data['mqtt']['password'] = os.environ.get('MQTT_PASSWORD', config_data['mqtt']['password'])
        config_data['mqtt']['use_tls'] = os.environ.get('MQTT_USE_TLS', str(config_data['mqtt']['use_tls'])).lower() in ('true', '1', 'yes')
        config_data['mqtt']['tls_ca_certs'] = os.environ.get('MQTT_TLS_CA_CERTS', config_data['mqtt']['tls_ca_certs'])
        config_data['mqtt']['tls_certfile'] = os.environ.get('MQTT_TLS_CERTFILE', config_data['mqtt']['tls_certfile'])
        config_data['mqtt']['tls_keyfile'] = os.environ.get('MQTT_TLS_KEYFILE', config_data['mqtt']['tls_keyfile'])
        config_data['mqtt']['tls_insecure'] = os.environ.get('MQTT_TLS_INSECURE', str(config_data['mqtt']['tls_insecure'])).lower() in ('true', '1', 'yes')
        config_data['mqtt']['tls_version'] = os.environ.get('MQTT_TLS_VERSION', config_data['mqtt']['tls_version'])
        config_data['mqtt']['tls_cert_reqs'] = os.environ.get('MQTT_TLS_CERT_REQS', config_data['mqtt']['tls_cert_reqs'])
        
        # RFID configuration
        config_data['rfid']['simulation_mode'] = os.environ.get('RFID_SIMULATION_MODE', str(config_data['rfid']['simulation_mode'])).lower() in ('true', '1', 'yes')
        config_data['rfid']['device_path'] = os.environ.get('RFID_DEVICE_PATH', config_data['rfid']['device_path'])
        
        # UI configuration
        config_data['ui']['fullscreen'] = os.environ.get('CONSULTEASE_FULLSCREEN', str(config_data['ui']['fullscreen'])).lower() in ('true', '1', 'yes')
        config_data['ui']['theme'] = os.environ.get('CONSULTEASE_THEME', config_data['ui']['theme'])
        
        # Keyboard configuration
        config_data['keyboard']['type'] = os.environ.get('CONSULTEASE_KEYBOARD', config_data['keyboard']['type'])
        
        # Logging configuration
        config_data['logging']['level'] = os.environ.get('LOG_LEVEL', config_data['logging']['level'])
        config_data['logging']['rfid_level'] = os.environ.get('LOG_RFID_LEVEL', config_data['logging']['rfid_level'])
    ...
    def get(self, key, default=None):
        ...
        if isinstance(value, str):
            ...
            try: return float(value) # For ports or other numbers
            except ValueError: pass # Not a float
    ...
    def save(self, config_path=None):
        ...
        if not config_path:
            ...
            existing_path_to_save = next((p for p in possible_paths if p and os.path.exists(p)), possible_paths[0])
