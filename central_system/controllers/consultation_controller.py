...
class ConsultationController:
    ...
    def __init__(self):
        ...
        if ConsultationController._instance is not None:
            raise RuntimeError(
                "ConsultationController is a singleton, use ConsultationController.instance()")
    ...
    def register_callback(self, callback):
        ...
        logger.info(
            f"Registered Consultation controller callback: {getattr(callback, '__name__', 'unnamed_callback')}")
    ...
    def unregister_callback(self, callback):
        ...
        try:
            if callback in self.callbacks:
                ...
                logger.info(
                    f"Unregistered Consultation controller callback: {getattr(callback, '__name__', 'unknown')}")
            else:
                logger.warning(
                    f"Attempted to unregister a callback that was not found: {getattr(callback, '__name__', 'unknown')}")
        except Exception as e:
            logger.error(
                f"Error unregistering Consultation controller callback {getattr(callback, '__name__', 'unknown')}: {str(e)}")
    ...
    def _ensure_mqtt_connected(self, operation_description: str = "operation") -> bool:
        ...
        if not self.mqtt_service.is_connected:
            logger.warning(
                f"MQTT not connected before {operation_description}. Attempting to connect.")
            ...
            while not self.mqtt_service.is_connected and (
                    time.time() - start_time) < connect_timeout:
                time.sleep(0.1)  # Brief pause before re-checking

            if not self.mqtt_service.is_connected:
                logger.error(
                    f"MQTT: Failed to connect within timeout before {operation_description}.")
                return False
            logger.info(f"MQTT: Successfully connected before {operation_description}.")
        return True

    @db_operation_with_retry()
    def _create_consultation_in_db(self, db, student_id, faculty_id,
                                   request_message, course_code=None):
        """
    ...
    def create_consultation(self, student_id, faculty_id, request_message, course_code=None):
        ...
        try:
            ...
            consultation = self._create_consultation_in_db(
                student_id, faculty_id, request_message, course_code)
            if not consultation:
            ...
            if not self._publish_consultation_to_mqtt(consultation.id):
                logger.warning(
                    f"Consultation created but failed to publish to MQTT: {consultation.id}")
    ...
    def _publish_consultation_to_mqtt(self, consultation_id):
        ...
        try:
            ...
            if not student or not faculty:
                logger.error(
                    f"MQTT Publish: Student or Faculty not found for Consultation {consultation_id}.")
                return False
            ...
            if consultation.course_code:
                message_for_display += f"Course: {consultation.course_code}\n"
            ...
            if success_specific:
                logger.info(f"MQTT: Published to specific topic {specific_topic}")
            else:
                logger.error(f"MQTT: Failed to publish to specific topic {specific_topic}")
            ...
            if success_legacy_raw:
                logger.info(f"MQTT: Published to legacy raw topic {legacy_topic}")
            else:
                logger.error(f"MQTT: Failed to publish to legacy raw topic {legacy_topic}")

            # Return true if primary specific publish worked, or if legacy fallback worked
            return success_specific or success_legacy_raw
        except Exception as e:
            logger.error(
                f"Error in _publish_consultation_to_mqtt for C_ID {consultation_id}: {str(e)}")
            return False
        finally:
            close_db()
    ...
    def update_consultation_status(self, db, consultation_id, status):
        ...
        if self._ensure_mqtt_connected(f"publishing status for C_ID {consultation.id}"):
            ...
            if publish_success_update:
                ...
                logger.error(
                    f"MQTT: Failed to publish status update for {consultation.id}. DB is updated.")
        else:
            # _ensure_mqtt_connected already logs the failure to connect
            logger.error(
                f"MQTT: Not connected. Status update for {consultation.id} saved in DB but not published.")
    ...
    def cancel_consultation(self, consultation_id):
        ...
        try:
            ...
            logger.error(
                f"Error in cancel_consultation controller method for C_ID {consultation_id}: {e}")
            return None
    ...
    def test_faculty_desk_connection(self, faculty_id):
        ...
        try:
            ...
            logger.info(
                f"Test message sent to faculty {faculty.name}. JSON Specific: {success_json}, Raw Legacy: {success_raw_legacy}")
    ...
    def accept_consultation_request(self, db, consultation_id: int, faculty_id: int):
        ...
        if consultation.faculty_id != faculty_id:
            logger.error(
                f"Accept failed: Faculty {faculty_id} not authorized for consultation {consultation_id} (belongs to faculty {consultation.faculty_id}).")
            return None

        if consultation.status != ConsultationStatus.PENDING:
            logger.warning(
                f"Consultation {consultation_id} is already in status {consultation.status.value}, cannot accept again.")
            # Optionally return the consultation if already accepted, or None/error if
            # it was cancelled/completed
        ...
        logger.info(
            f"Consultation {consultation_id} status updated to ACCEPTED by faculty {faculty_id}.")

        # Re-fetch for callbacks to ensure relationships are loaded
        loaded_consultation = db.query(Consultation).options(joinedload(
            '*_related')).filter(Consultation.id == consultation.id).first()
    ...
    def reject_consultation_request(self, db, consultation_id: int, faculty_id: int):
        ...
        if consultation.faculty_id != faculty_id:
            logger.error(
                f"Reject failed: Faculty {faculty_id} not authorized for consultation {consultation_id} (belongs to faculty {consultation.faculty_id}).")
            return None

        if consultation.status not in [ConsultationStatus.PENDING, ConsultationStatus.ACCEPTED]:
            logger.warning(
                f"Consultation {consultation_id} is in status {consultation.status.value}, cannot reject if not PENDING or already ACCEPTED (to change mind). Current status: {consultation.status.value}")
            # If it's already completed or cancelled, can't reject.
            if consultation.status in [ConsultationStatus.COMPLETED,
                                       ConsultationStatus.CANCELLED_BY_STUDENT, ConsultationStatus.CANCELLED_BY_FACULTY]:
                return None
            # Allow rejection if it was PENDING or ACCEPTED (faculty changed their
            # mind before starting)

        consultation.status = ConsultationStatus.REJECTED_BY_FACULTY  # Assuming this enum value exists
        # db.commit() is handled by the decorator
        logger.info(
            f"Consultation {consultation_id} status updated to REJECTED_BY_FACULTY by faculty {faculty_id}.")
    ...
    def start_consultation(self, db, consultation_id: int, faculty_id: int):
        ...
        if consultation.faculty_id != faculty_id:
            logger.error(
                f"Start failed: Faculty {faculty_id} not authorized for consultation {consultation_id}.")
            return None
        if consultation.status != ConsultationStatus.ACCEPTED:
            logger.warning(
                f"Cannot start consultation {consultation_id}: current status is {consultation.status.value}, not ACCEPTED.")
            return None

        consultation.status = ConsultationStatus.STARTED
        # Optionally set a started_at timestamp if your model has one
        logger.info(
            f"Consultation {consultation_id} status updated to STARTED by faculty {faculty_id}.")
        loaded_consultation = db.query(Consultation).options(joinedload(
            '*_related')).filter(Consultation.id == consultation.id).first()
        self._notify_callbacks(loaded_consultation if loaded_consultation else consultation)
    ...
    def complete_consultation(self, db, consultation_id: int, faculty_id: int):
        ...
        if consultation.faculty_id != faculty_id:
            logger.error(
                f"Complete failed: Faculty {faculty_id} not authorized for consultation {consultation_id}.")
            return None
        ...
        if consultation.status not in [ConsultationStatus.STARTED, ConsultationStatus.ACCEPTED]:
            logger.warning(
                f"Cannot complete consultation {consultation_id}: current status is {consultation.status.value}, not STARTED or ACCEPTED.")
            return None

        consultation.status = ConsultationStatus.COMPLETED
        consultation.completed_at = datetime.datetime.now()
        logger.info(
            f"Consultation {consultation_id} status updated to COMPLETED by faculty {faculty_id}.")
        loaded_consultation = db.query(Consultation).options(joinedload(
            '*_related')).filter(Consultation.id == consultation.id).first()
        self._notify_callbacks(loaded_consultation if loaded_consultation else consultation)
    ...
    def cancel_consultation_by_faculty(self, db, consultation_id: int, faculty_id: int):
        ...
        if consultation.faculty_id != faculty_id:
            logger.error(
                f"Faculty cancel failed: Faculty {faculty_id} not authorized for consultation {consultation_id}.")
            return None

        # Prevent cancelling if already completed or cancelled by student
        if consultation.status in [ConsultationStatus.COMPLETED,
                                   ConsultationStatus.CANCELLED_BY_STUDENT, ConsultationStatus.REJECTED_BY_FACULTY]:
            logger.warning(
                f"Cannot cancel consultation {consultation_id}: current status is {consultation.status.value}.")
        ...
        logger.info(
            f"Consultation {consultation_id} status updated to CANCELLED_BY_FACULTY by faculty {faculty_id}.")
        loaded_consultation = db.query(Consultation).options(joinedload(
            '*_related')).filter(Consultation.id == consultation.id).first()
        self._notify_callbacks(loaded_consultation if loaded_consultation else consultation)
    ...
    def _notify_student_of_status_change(self, consultation: Consultation):
        ...
        if not consultation or not consultation.student_id or not consultation.faculty:
            logger.warning(
                f"_notify_student_of_status_change: Missing data for consultation {consultation.id if consultation else 'Unknown'}")
            return
        ...
        try:
            ...
            logger.info(
                f"Notifying student {consultation.student_id} on topic {topic} with payload: {payload}")
            # Use QoS 1 for important notifications
            self.mqtt_service.publish(topic, payload, qos=1)
        except Exception as e:
            logger.error(
                f"Error in _notify_student_of_status_change for C_ID {consultation.id}: {e}",
                exc_info=True)
